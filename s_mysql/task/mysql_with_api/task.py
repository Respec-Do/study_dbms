...
from s_mysql.task.mysql_with_api.sms.mail_module import *
from papago import *
from ocr import *
...
if __name__ == '__main__':
    ...
    message = "한국어 입력하세요: "
    user_kor = input(message)
    translated_user_kor = papago(user_kor)
    save_query = "insert into tbl_text_content(kor_text, translated_text,email) values(%s, %s, %s)"
    save_params = (user_kor, translated_user_kor,member_email)
    save(save_query, save_params)
    ...
    # 경로와 추출한 텍스트 전체 조회
    image_name = input('이미지의 이름: \n')
    upload_image = 'https://thumb.mt.co.kr/06/2012/02/2012021613230156226_1.jpg/dims/optimize/'
    translated_to_text = ocr(upload_image)
    save_query = "insert into tbl_image_content(name, translated_text, path) values(%s, %s, %s)"
    save_params = (image_name, translated_to_text, upload_image)
    save(save_query, save_params)